        ...
        <div className="flex justify-between items-center mb-8 border-b border-outline-variant/30 pb-5">
          <div>
            <h1 className="font-headline text-xl md:text-2xl font-black text-on-surface uppercase tracking-tight">
              Quản lý sản phẩm (Mock CRUD)
            </h1>
            <p className="text-xs text-on-surface-variant font-medium mt-1">
              Admin console / Danh sách sản phẩm lưu tại localStorage
            </p>
          </div>
              ...
              {products.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-10 text-on-surface-variant">
                    Chưa có sản phẩm nào. Hãy bấm "Thêm sản phẩm" để khởi tạo.
                  </TableCell>
                </TableRow>
              ) : (
                  ...
                  <TableRow key={product.id} className="border-b border-outline-variant/20 hover:bg-surface-container-low/50">
                    <TableCell className="py-3">
                      ...
                      <img
                        ...
                        alt={product.name}
                        className="w-10 h-12 object-cover rounded bg-surface-container-low"
                      />
                    </TableCell>
                    <TableCell className="py-3">
                    ...
                    <TableCell className="py-3 text-right">
                      {product.stock <= 10 ? (
                        <Badge variant="destructive" className="font-semibold text-[10px] py-0 px-1.5">{product.stock} (Sắp hết)</Badge>
                      ) : (
                    ...
                    <TableCell className="py-3">
                      <div className="flex items-center justify-center gap-2">
                        <Link href={`/products/${product.slug}`} target="_blank" title="Xem trên web">
                          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-surface-container text-on-surface-variant cursor-pointer">
                            <ExternalLink className="size-4" />
                          </Button>
                        </Link>
                        <Link href={`/admin/products/${product.id}/edit`} title="Chỉnh sửa">
                          <Button variant="ghost" size="icon" className="h-8 w-8 hover:bg-surface-container text-primary cursor-pointer">
