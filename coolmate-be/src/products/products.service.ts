...
export class ProductsService {
  async findAll(query: ProductQueryDto) {
    ...
    if (query.category) {
      const [category] = await db
        ...
        .limit(1);
      if (!category) return this.paginatedResponse([], page, limit, 0);
    ...
    const [items, [{ value: total }]] = await Promise.all([
      db.query.products.findMany({
        where,
        with: { category: true, images: true, options: true },
        limit,
        offset: (page - 1) * limit,
      }),
    ...
    const mappedItems = items.map((item) =>
      query.view === 'card' ? this.mapProductCard(item) : this.mapProduct(item),
    );

    return this.paginatedResponse(mappedItems, page, limit, total);
  }
  ...
  async update(id: string, dto: UpdateProductDto) {
    ...
    if (dto.categoryId || dto.category)
      values.categoryId = await this.resolveCategoryId(
        dto.categoryId,
        dto.category,
      );
    ...
    if (dto.originalPrice !== undefined)
      values.originalPrice = dto.originalPrice;
  ...
  private mapProductCard(product: ProductWithRelations) {
    const baseProduct = this.mapProduct(product);
    return {
      id: baseProduct.id,
  ...
  private mapProduct(product: ProductWithRelations) {
    const colors = Array.from(
      new Map(
        product.options.map((option) => [
          option.colorValue,
          { name: option.colorName, value: option.colorValue },
        ]),
      ).values(),
    );
    ...
    return {
      ...
      sizes: [...new Set(product.options.map((option) => option.size))],
      colors: colors.map((color) => color.name),
      options: product.options.map((option) => ({
        id: option.id,
        size: option.size,
        color: { name: option.colorName, value: option.colorValue },
        stock: option.stock,
      })),
      stock: product.stock,
